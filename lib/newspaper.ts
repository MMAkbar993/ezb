...
// -- Email rendering ------------------------------------------------------------
// Moved to lib/newspaperRender.ts (no 'use client' directive) so the server-
// only publish route can actually import it — see that file's header comment
// for why this file's own 'use client' directive broke that import silently.
// Re-exported here so existing client-side imports of renderNewspaperHtml
// from '@/lib/newspaper' (the dashboard preview tab) keep working unchanged.
export { renderNewspaperHtml } from './newspaperRender'
